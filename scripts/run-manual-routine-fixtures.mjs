...
const resumed=prepareResume(previous,state,[...retained],sourceHash);
state=resumed.state;
const trace=resumed.trace;
let scenario='';
...
const results=resumed.results.map(r=>({...r,retainedFrom:resumePath,sourceHash:previous.sourceHash}));
async function run(id,prompt,check){
 if(resumed.skip.has(id))return;
 ...
 results.push({id,passed,error,...(passed&&retained.has(id)?{stateAfter:structuredClone(state)}:{}),activeSkill,activeSourceHash:createHash('sha256').update(activeSource).digest('hex'),sourceHash,executedAt:new Date().toISOString(),toolCalls:trace.length-start,text:result.text});
...
await run('independent-learning','Run only the learning/reconciliation step. The fixture human has now answered. Do not prepare new iterations. Refresh current iteration results and opportunity metrics where supported.',calls=>{for(const p of ['alpha','connectors']){const iteration=state.iterations[`${p}:1`];const completion=calls.findIndex(c=>c.name==='notion_save_iteration'&&c.args.profile===p&&c.args.status==='Complete');assert.ok(completion>=0,`${p}: missing completion write`);const answers=state.lists[iteration.list_id].answers;assert.deepEqual(answers.map(a=>a.relationshipId).sort(),[...iteration.candidate_ids].sort(),`${p}: answers must match saved batch`);assertReviewEvidence(calls.slice(0,completion),iteration.list_id,answers);}assert.equal(state.iterations['alpha:1'].status,'Complete');assert.equal(state.iterations['alpha:1'].yes_rate,.4);assert.ok(state.iterations['alpha:1'].learning);assert.equal(state.iterations['beta:1'].status,'To review');assert.equal(state.iterations['connectors:1'].status,'Complete');assert.equal(Object.keys(state.iterations).length,3);});
}else{
...
assert.equal(results.length,7,'All seven scenarios must have results');
assert.ok(results.every(r=>r.passed===true),'Cannot report success with failed retained results');
